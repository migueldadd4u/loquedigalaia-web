...
for (const fuente of fuentes) {
  ...
  for (const [k, v] of Object.entries(dict)) {
    ...
    // Las cifras no pueden perderse ni cambiar de valor. No se exige el mismo
    // multiconjunto: hay idiomas que reescriben las fechas («10 de enero de 1991»
    // → «1991 年 1 月 10 日») y repiten o reordenan números legítimamente.
    const perdidas = cifras(k).filter((n) => !cifras(v).includes(n));
